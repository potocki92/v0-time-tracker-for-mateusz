...
function NavRow({
  ...
  const showBadgeLabel = Boolean(item.badge?.labelKey)
...
function BadgePill({ badge }: { badge: NavBadge }) {
  ...
  return (
    <span className={cn(BADGE_BASE, 'uppercase')}>
      {badge.labelKey ? t(badge.labelKey) : null}
    </span>
  )
}
...
function QuickActionButton({ item }: { item: NavItem }) {
  ...
  const { href, labelKey } = item.quickAction
...
function PinnedRow({ item }: { item: PinnedItem }) {
  // Nazwa przypietego projektu to DANA uzytkownika — nie tlumaczymy jej.
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        asChild
