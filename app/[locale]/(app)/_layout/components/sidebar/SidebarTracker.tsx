...
export function SidebarTracker({ label }: SidebarTrackerProps) {
  const t = useTranslations('navigation')
  const trackerLabel = label ?? t('tracker.defaultLabel')
  ...
  if (isCollapsed) {
    return (
      <button
        type="button"
        onClick={toggle}
        title={`${trackerLabel} • ${formatHMS(tick)}`}
  ...
  return (
    ...
    >
      <div className="mb-1.5 flex items-center gap-2">
        ...
        />
        <span className="truncate text-2xs text-sidebar-foreground/70">{trackerLabel}</span>
      </div>
