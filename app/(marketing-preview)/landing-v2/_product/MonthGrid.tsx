...
import { useScrollMap } from '../_motion/scene'
  ...
  return (
    <div className={fill ? 'flex h-full flex-col' : undefined}>
      <div className="mb-1 grid grid-cols-7 gap-1">
        {DEMO_WEEKDAY_SHORT.map((label) => (
          <span key={label} className="text-center lv2-t8 uppercase tracking-wide text-zinc-400">
            {label}
          </span>
        ))}
      </div>
...
function DayCell({
  ...
  return (
    <div
      className={[
        'lv2-day',
        weekend && !worked ? 'lv2-day-weekend' : '',
        day.inTrip ? 'lv2-day-trip' : '',
      ]
        .filter(Boolean)
        .join(' ')}
    >
      {/* Skorka wpisu jest osobna warstwa sterowana tym samym postepem, co
          ...
          cokolwiek dopisze. */}
      {worked && <m.span aria-hidden className="lv2-day-fill" style={{ opacity }} />}

      <span
        className={
          isToday
            ? 'relative flex size-3.5 items-center justify-center rounded-full bg-[var(--lv2-accent)] lv2-t8 font-semibold text-black'
            : 'relative lv2-t8 font-semibold leading-none text-zinc-400'
        }
      >
        {day.day}
      </span>
      ...
      {worked && (
        <m.span
          ...
          style={{ opacity, scale }}
        >
          <span className="block truncate lv2-t9 font-bold leading-tight text-white">
            {formatHours(day.hours)}
          </span>
          {showAmount && (
            <span className="hidden truncate lv2-t8 leading-tight text-zinc-400 sm:block">
              {formatMoney(toMinor((day.hours ?? 0) * DEMO_RATE_EUR), 'EUR')}
            </span>
          )}
          <span
            aria-hidden
