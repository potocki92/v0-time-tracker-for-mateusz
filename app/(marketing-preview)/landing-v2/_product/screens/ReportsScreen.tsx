    ...
    <div className="flex h-full flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <span className="rounded-md border border-[var(--lv2-hair-2)] px-1.5 py-0.5 lv2-t8 text-zinc-400">
            Eksport CSV
          </span>
          <span className="rounded-md border border-[var(--lv2-hair-2)] px-1.5 py-0.5 lv2-t8 text-zinc-400">
            PDF
          ...
          {DEMO_PROJECTS.filter((project) => project.hours > 0).map((project) => {
            ...
            return (
              <li key={project.id} className="space-y-1">
                <div className="flex items-center gap-1.5 lv2-t9">
                  <Dot color={demoClient(project.clientId).color} />
                  <span className="min-w-0 flex-1 truncate text-zinc-200">{project.name}</span>
                  <span className="lv2-mono tabular-nums text-zinc-300">
                    {formatHours(project.hours)}
                  </span>
                  <span className="lv2-mono w-8 text-right tabular-nums text-zinc-400">
                    {formatPercent(share)}
                  </span>
                </div>
