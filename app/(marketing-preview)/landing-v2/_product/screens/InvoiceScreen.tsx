  ...
  return (
    <div className="flex h-full flex-col gap-2">
      <Card className="flex min-h-0 flex-1 flex-col">
        <header className="flex items-start justify-between gap-2 border-b border-[var(--lv2-hair)] pb-2">
          <div>
            <p className="lv2-mono lv2-t12 font-semibold text-white">{DEMO_INVOICE.number}</p>
            <p className="lv2-t9 text-zinc-400">Okres: {DEMO_INVOICE.period}</p>
          </div>
          <div className="text-right">
            <Pill>{DEMO_INVOICE.status}</Pill>
            <p className="mt-1 lv2-t8 text-zinc-400">
              Wystawiona {DEMO_INVOICE.issueDate} · termin {DEMO_INVOICE.dueDate}
            </p>
          </div>
        </header>

        <div className="grid grid-cols-2 gap-2 border-b border-[var(--lv2-hair)] py-2">
          <div>
            <Eyebrow>Sprzedawca</Eyebrow>
            <p className="mt-1 lv2-t9 text-zinc-300">Mateusz Potocki</p>
          </div>
          <div>
            <Eyebrow>Nabywca</Eyebrow>
            <p className="mt-1 lv2-t9 text-zinc-300">{client.name}</p>
          </div>
        </div>
        ...
        <table className="mt-2 w-full">
          ...
          <tbody>
            <tr className="lv2-t9 text-zinc-200">
              <td className="py-1.5">
                {project.name}
                <span className="block lv2-t8 text-zinc-400">
              ...
              <td className="lv2-mono py-1.5 text-right tabular-nums text-white">
        ...
        <div className="mt-auto space-y-1 border-t border-[var(--lv2-hair)] pt-2 lv2-t9">
          ...
          />
          <div className="flex items-center justify-between border-t border-[var(--lv2-hair)] pt-1.5">
            <span className="lv2-t10 font-medium text-white">Razem do zapłaty</span>
            <span className="lv2-mono lv2-t13 font-semibold tabular-nums text-[var(--lv2-accent)]">
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between text-zinc-400">
      <span>{label}</span>
      <span className="lv2-mono tabular-nums">{value}</span>
    </div>
  )
}
