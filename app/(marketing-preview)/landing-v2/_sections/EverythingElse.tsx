...
export function EverythingElse() {
  return (
    <section aria-labelledby="everything-heading" className="mx-auto max-w-[1400px] px-5 py-24 sm:px-8 sm:py-32">
      <span className="lv2-eyebrow">Everything else</span>
      <h2 id="everything-heading" className="lv2-display lv2-d2 mt-3 max-w-[16ch]">
        Built for independent work.
      </h2>

      <ul className="mt-12 grid gap-x-10 gap-y-8 sm:grid-cols-2 lg:grid-cols-3">
        {ITEMS.map((item) => (
          <li key={item.title} className="border-t border-[var(--lv2-hair)] pt-4">
            <h3 className="text-sm font-medium text-white">{item.title}</h3>
            <p className="mt-1.5 text-sm leading-relaxed text-[var(--lv2-ink-2)]">{item.body}</p>
          </li>
        ))}
      </ul>
    </section>
