...
import { useScrollMap, useTrackProgress } from '../_motion/scene'
import { MonthGrid } from '../_product/MonthGrid'
...
export function AutomationShowcase({ month }: { month: DemoMonth }) {
  ...
  return (
    <section id="automation" aria-labelledby="automation-heading">
      <div ref={trackRef} className="lv2-track relative h-[260vh] lg:h-[340vh]">
        <div className="lv2-stage sticky top-0 flex h-[100svh] items-center">
          <div className="mx-auto w-full max-w-[1400px] px-5 sm:px-8">
            <span className="lv2-eyebrow">Automation</span>
            <h2 id="automation-heading" className="lv2-display lv2-d2 mt-3 max-w-[16ch]">
              Your calendar can fill itself.
            </h2>
            <p className="mt-4 max-w-[62ch] text-sm leading-relaxed text-[var(--lv2-ink-2)] sm:text-base">
            ...
            <div className="mt-5 grid gap-3 sm:mt-8 lg:grid-cols-[minmax(0,300px)_minmax(0,1fr)] lg:gap-8">
              <div className="grid gap-3 sm:grid-cols-2 lg:block lg:space-y-4">
                <div className="lv2-card p-4">
                  <p className="lv2-eyebrow">Grafik tygodnia</p>
                  ...
                  <ul className="mt-3 grid grid-cols-7 gap-1 lg:block lg:space-y-1.5">
                    {DEMO_WEEKDAY_KEYS.map((key, index) => {
                      ...
                      return (
                        ...
                        >
                          <span className="text-[var(--lv2-ink-3)] lg:w-8">
                            {DEMO_WEEKDAY_SHORT[index]}
                          </span>
                          <span className="hidden h-px flex-1 bg-white/10 lg:block" />
                          <span
                            className={`lv2-mono tabular-nums ${
                              plan.enabled ? 'text-white' : 'text-[var(--lv2-ink-3)]'
                            }`}
                          >
                ...
                <div className="lv2-card p-4">
                  <p className="lv2-eyebrow">Obecność</p>
                  <ul className="mt-3 space-y-2">
                    {DEMO_PRESENCE.map((entry) => (
                      <li key={entry.range} className="flex items-center gap-2.5 text-xs">
                        <span
                          className={`h-6 w-1 rounded-full ${
                            entry.kind === 'trip' ? 'bg-[var(--lv2-accent)]' : 'bg-white/15'
                          }`}
                        />
                        <span className="flex-1 text-[var(--lv2-ink-2)]">{entry.label}</span>
                        <span className="lv2-mono tabular-nums text-[var(--lv2-ink-3)]">
                          {entry.range}
                        </span>
                      </li>
                    ))}
              ...
              <div className="lv2-card p-4">
                <div className="flex items-baseline justify-between">
                  ...
                  <m.p
                    className="lv2-motion lv2-mono text-sm tabular-nums text-[var(--lv2-accent)]"
                ...
                <ul className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-[var(--lv2-hair)] pt-3">
                  {reasons.map((reason) => (
                    <li key={reason} className="lv2-t11 text-[var(--lv2-ink-3)]">
