'use client'

import Link from 'next/link'
import { useState } from 'react'
import { m, useMotionValueEvent, useScroll } from 'framer-motion'

import { useScrollMap } from '../_motion/scene'

const LINKS = [
  { label: 'Product', href: '#product' },
  { label: 'Automation', href: '#automation' },
  { label: 'How it works', href: '#flow' },
] as const

/**
 * Navbar: logo, trzy kotwice, dwie akcje.
 *
 * Tlo i obrys jada na MotionValue (zero rerenderow), a `backdrop-filter`
 * wchodzi dopiero po przekroczeniu progu — jako klasa, nie jako animowana
 * wlasciwosc. Animowany blur to najdrozsza rzecz, jaka mozna wpisac do
 * przyklejonego paska.
 */
export function NavbarV2() {
  const { scrollY } = useScroll()
  const [scrolled, setScrolled] = useState(false)

  const background = useScrollMap(scrollY, [0, 80], ['rgba(0,0,0,0)', 'rgba(0,0,0,0.72)'])
  const borderColor = useScrollMap(
    scrollY,
    [0, 80],
    ['rgba(255,255,255,0)', 'rgba(255,255,255,0.10)'],
  )

  useMotionValueEvent(scrollY, 'change', (value) => setScrolled(value > 24))

  return (
    <m.header
      className={`fixed inset-x-0 top-0 z-50 border-b ${scrolled ? 'backdrop-blur-md' : ''}`}
      style={{ background, borderColor }}
    >
      <nav
        aria-label="Główna"
        className="mx-auto flex h-14 max-w-[1400px] items-center gap-3 px-5 sm:px-8"
      >
        <Link href="/landing-v2" className="flex items-center gap-2">
          <span aria-hidden className="size-4 rounded-[5px] bg-[var(--lv2-accent)]" />
          <span className="lv2-t13 font-semibold tracking-tight">TimeTracker</span>
        </Link>

        <span className="lv2-pill hidden sm:inline-flex">Preview</span>

        <div className="ml-auto hidden items-center gap-1 md:flex">
          {LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="rounded-lg px-3 py-2 lv2-t13 text-[var(--lv2-ink-2)] transition-colors hover:text-[var(--lv2-ink-1)]"
            >
              {link.label}
            </a>
          ))}
        </div>

        <div className="ml-auto flex items-center gap-2 md:ml-4">
          <Link
            href="/auth/login"
            className="rounded-lg px-3 py-2 lv2-t13 text-[var(--lv2-ink-2)] transition-colors hover:text-[var(--lv2-ink-1)]"
          >
            Sign in
          </Link>
          <Link href="/dashboard" className="lv2-cta !min-h-9 !px-4 !lv2-t13">
            Open app
          </Link>
        </div>
      </nav>
    </m.header>
  )
}
