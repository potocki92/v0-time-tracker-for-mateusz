 ...
 *
 * Waluta NIE zalezy od jezyka interfejsu: 100 EUR zostaje 100 EUR takze po
 * przelaczeniu UI na polski. Zmienia sie wylacznie zapis liczby i pozycja
 * symbolu ("1 234,56 €" / "1.234,56 €" / "€1,234.56").
 */
...
export function formatMoney(
  ...
  return numberFormatter(locale, { ...currencyOptions(currency), ...TWO_DECIMALS }).format(
    toMajor(minor),
  )
}
...
export function formatRate(
  ...
  const amount = formatMoney(locale, minorPerHour, currency)
